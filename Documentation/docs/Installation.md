...
!!! note
	...
	In any case, ReJig does not use the CSD Python API at all. Only [ACSD](https://geoffreyweal.github.io/ACSD) reads directly from the CSD. 
...
Once you have run ``source ~/.bashrc``, the ReJig python package is on your ``PYTHONPATH``, so you can ``import ReJig`` from your own scripts.

That does not give you the ``rejig`` terminal command. ``rejig`` is generated when the package is installed, rather than shipped as a file inside the repository, so a cloned copy alone does not provide it. To create it from your clone, run an editable install from inside the ReJig folder:
...
This should give you the path to the ``rejig`` command. If the terminal tells you it can not find it, check that the editable install above completed, and that the path you gave for ``PATH_TO_ReJig`` is the correct path to the ReJig folder. 
