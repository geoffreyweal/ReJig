...
def _get_molecule_spec(atoms, nuclear_props):
    ...
    for constraint in atoms.constraints:
        if isinstance(constraint,ase.constraints.FixAtoms):
            constrain_atoms += constraint.index.tolist()
    constrain_atoms = set(constrain_atoms)
    ...
    for i, atom in enumerate(atoms):
        ...
        # Freeze atom if desired
        constrain_atom = '-1' if (i in constrain_atoms) else ''
        ...
        if len(constrain_atoms) > 0:
            molecule_spec.append('{:<10s}{:<5s}{:20.10f}{:20.10f}{:20.10f}'.format(symbol_section, constrain_atom, *atom.position))
        else:
...
def write_gaussian_in(fd, atoms, perform_opt, perform_CalcAll, perform_TD, 
    ...
    # GRW Modification: only allow lowest multiplicity possible
    if mult is None:
        mult = (atoms.get_initial_magnetic_moments().sum() % 2.0) + 1
    ...
    header_of_file = 'ReJig Geometric Optimsation for molecule '+str(molecule_name)
